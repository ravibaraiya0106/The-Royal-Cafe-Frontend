...
const DeliveryDashboard = () => {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      const [activeRes, completedRes] = await Promise.all([
        getMyDeliveriesService({ status: "active", limit: 5 }),
        getMyDeliveriesService({ status: "completed", limit: 5 }),
      ]);
  ...
  const handleStatusUpdate = async (id: string, nextStatus: string) => {
    try {
      ...
      toastSuccess(`Order status updated to ${nextStatus.replace(/_/g, " ")}`);
      fetchDashboardData();
    } catch (err: unknown) {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <div>
            ...
            <p className="text-xs sm:text-sm text-gray-500 mt-1">
              Manage your active delivery assignments and track your daily performance.
            </p>
          </div>
          <button
            onClick={fetchDashboardData}
            disabled={loading}
            className="self-start sm:self-auto px-4 py-2 bg-white hover:bg-gray-50 text-brand rounded-lg text-xs font-semibold border border-gray-300 flex items-center gap-2 transition-all shadow-sm disabled:opacity-50"
          >
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-4">
          <div className="flex items-center justify-between border-b border-gray-100 pb-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-gray-900">Active Delivery Tasks</h2>
            </div>
            <Link
            ...
            <div className="space-y-4">
              {activeTasks.map((task) => {
                ...
                return (
                  <div
                    key={task._id}
                    className="bg-gray-50 border border-gray-200 rounded-xl p-5 hover:border-brand/40 transition-all shadow-sm"
                  >
                    ...
                    <div className="pt-3 border-t border-gray-200 flex items-center justify-end gap-3">
                      ...
                      {(status === "out_for_delivery" || status === "picked" || status === "assigned") && (
                        <button
                          ...
                          disabled={updatingId === task._id}
                          className="px-4 py-2 bg-brand hover:bg-brand/90 text-white font-semibold text-xs rounded-lg shadow-sm transition-all disabled:opacity-50"
                        >
                          Complete Delivery
                        </button>
                      )}
